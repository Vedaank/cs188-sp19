...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        choosePositions = list()
        for action in legal:
            nextPos = Actions.getSuccessor(pacmanPosition, action)
            ghostPositions = list()
            for ghostPosDist in livingGhostPositionDistributions:
                ghostPositions.append(self.distancer.getDistance(nextPos, ghostPosDist.argMax()))
            choosePositions.append((action, min(ghostPositions))) 
        return min(choosePositions, key = lambda x : x[1])[0]
